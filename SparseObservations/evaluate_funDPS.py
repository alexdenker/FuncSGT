...
def main(args):
    ...
    score_model = ScoreModel(
        model=model, sde=sde, noise_sampler=noise_sampler, model_type=model_type
    )

    def sample_conditional(y, forward_op, num_samples_K, pos):
        """
    ...
    noisy_observations = torch.from_numpy(
        np.load("test_data/noisy_observations.npy")
    ).to(device)
    ...
    for dataset_idx in range(num_dataset_elements):
        ...
        samples = sample_conditional(y_noise, solver, num_samples, pos).squeeze(1)
