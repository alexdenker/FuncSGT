...
conditioning_operator = Conditioning(nx=n_points, sigma=forward_op_config['cond_sigma'], device=device)
...
model.load_state_dict(torch.load(os.path.join( f"unconditional_model_fno/model_type={model_type}/alpha={power}/", "ema_model.pt"), weights_only=False)["shadow"])
model.to(device)
model.eval()

print("NUMBER OF PARAMETERS in base model:", sum(p.numel() for p in model.parameters() if p.requires_grad))
...
h_trans.load_state_dict(torch.load(os.path.join(load_path_htrans, "ema_model.pt"), weights_only=False)["shadow"])
h_trans.to(device)
h_trans.eval()
...
for dataset_idx in range(num_dataset_elements):
    ...
    samples = sample_conditional(y=y_noise, 
                                 num_samples_K=num_samples,
                                 pos=pos, 
                                 forward_op=solver).squeeze(1)
    ...
    ax.scatter(solver.eval_points.cpu().numpy(), y_noise.cpu().numpy().flatten(), 
               color='red', s=20, label='Measurement Points', zorder=5)
    # Plot samples
