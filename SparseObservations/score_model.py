...
class HtransformModel:
    def __init__(self, h_trans, model, sde, noise_sampler, model_type, cond_model_type, cond_operator):
    ...
    def __call__(self, x, y, t, grid, forward_op):
        ...
        v, m = self.cond_operator(eval_points, observations)  # v,m: [batch_size, n_points]
        v = v.unsqueeze(1)  # [batch_size, 1, n_points]
        m = m.unsqueeze(1)  # [batch_size, 1, n_points]
        ...
        pred_h = self.h_trans(x=x, 
                              x0hat=x0hat, 
                              y=y_inp, 
                              log_grad=log_likelihood_grad, 
                              pos=grid, 
                              t=t).unsqueeze(1) / std_t    
