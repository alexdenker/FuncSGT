...
class SpectralConv1d(nn.Module):
    ...
    def forward(self, x, emb):
        """
        The function uses zero padding to reduce boundary artifacts.
        ...
        # Apply zero padding to reduce boundary artifacts
        pad_size = L // 4  # Pad by 25% on each side
        x_padded = F.pad(x, (pad_size, pad_size), mode="constant", value=0)

        x_ft = torch.fft.rfft(x_padded, norm="ortho")  # (B, C_in, L_padded//2+1)
        ...
        out_ft[:, :, : self.modes] = self.compl_mul1d(
            x_ft[:, :, : self.modes], self.weights_pos
        ) * phi_complex_pos.unsqueeze(
            1
...
class FNO(nn.Module):
    def __init__(
        self, modes, width, n_layers, timestep_embedding_dim=33, max_period=2.0
    ):
        ...
        for _ in range(self.n_layers):
            self.layers.append(
                FNOBlock(self.modes, self.width, self.timestep_embedding_dim)
            )
    ...
    def forward(self, x, pos, t):
        ...
        emb = timestep_embedding(
            t, self.timestep_embedding_dim, max_period=self.max_period
        )
...
class ConditionalFNO(nn.Module):
    def __init__(
        ...
        self.fc0 = nn.Linear(4, self.width)  # input [x, v, w, pos]
        self.fc1 = nn.Linear(self.width, 128)
        self.fc2 = nn.Linear(128, 1)
    ...
    def forward(self, x, y, pos, t):
        """
        x: [batch, 1, num_points] - 1D signal
        y: [batch, 2, num_points] - 1D signal
        pos: [batch, 1, num_points] - position encoding
        t: [batch] - timesteps
...
class CondFNO(nn.Module):
    ...
    def forward(self, x, x0hat, y, log_grad, pos, t):
        """
        x: [batch, 1, num_points] - 1D signal
        ...
        """
        # Concatenate x and pos as two channels: [batch, 2, num_points]
        x = torch.cat([x, x0hat, y, pos], dim=1)  # [batch, 5, num_points]
    ...
    def scaling_network(self, t):
        """
        t: [batch] - timesteps

        Returns: [batch, 1] scaling factor
        """
        emb = timestep_embedding(
            t, self.timestep_embedding_dim, max_period=self.max_period
        )
        scaling = self.log_likelihood_scaling(emb)  # [batch, 1]
        return scaling  # [batch, 1]
