...
class PointEvaluationOperator(nn.Module):
    ...
    def __init__(self, nx=64, n_points=10, sigma=0.01, seed=None, device='cpu'):
        ...
        self.eval_points = torch.rand(n_points, device=device, generator=generator).sort()[0]
...
if __name__ == "__main__":
    ...
    operator = PointEvaluationOperator(nx=128, n_points=10, sigma=0.02, seed=42, device=device)
    ...
    plt.plot(x.cpu().numpy().flatten(), u0.cpu().numpy().flatten(), 
            label='Function $u(x)$', linewidth=2)
    plt.scatter(operator.eval_points.cpu().numpy(), measurements.cpu().numpy().flatten(), 
               color='red', s=50, label='Measurement Points', zorder=5)
    plt.plot(x.cpu().numpy().flatten(), v.cpu().numpy().flatten(), 
            label='Conditioned $v(x)$', linestyle='--', linewidth=2)
    plt.plot(x.cpu().numpy().flatten(), m.cpu().numpy().flatten(), 
            label='Mask $m(x)$', linestyle=':', linewidth=2)
    ...
    plt.subplot(1, 3, 1)
    plt.plot(x.cpu().flatten(), u0.cpu().flatten(), 'k--', linewidth=2, label='True Function $u(x)$')
    plt.scatter(operator.eval_points.cpu().numpy(), y_obs.cpu().numpy().flatten(), 
               color='red', s=50, label='Noisy Measurements', zorder=5)
    plt.scatter(operator.eval_points.cpu().numpy(), measurements.cpu().numpy().flatten(), 
               color='green', s=30, label='Clean Measurements', marker='x', zorder=4)
    ...
    plt.subplot(1, 3, 2)
    plt.plot(x.cpu().flatten(), u0.cpu().flatten(), 'k--', linewidth=2, label='True Function $u(x)$')
    plt.plot(x.cpu().flatten(), u_hat.detach().cpu().flatten(), 'b-', linewidth=2, label='Reconstructed $\hat{u}(x)$')
    plt.scatter(operator.eval_points.cpu().numpy(), y_obs.cpu().numpy().flatten(), 
               color='red', s=50, label='Noisy Measurements', zorder=5)
