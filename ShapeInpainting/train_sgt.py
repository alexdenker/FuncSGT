...
optimizer = torch.optim.Adam(
    cond_model.parameters(), lr=float(config["training"]["learning_rate"])
)
...
for epoch in range(num_epochs):
    ...
    for idx, batch in tqdm(enumerate(data_loader), total=len(data_loader)):
        ...
        cond_fourier = x_fourier[
            :, :, :num_cond_coeffs, :
        ]  # (batch_size, 2, num_cond_coeffs, 2)
    ...
    if epoch % 50 == 0 and epoch > 0:
        ...
        cond_fourier = fourier_coefficients(x_gt, num_bases=num_fourier_modes)[
            :, :, :num_cond_coeffs, :
        ]
