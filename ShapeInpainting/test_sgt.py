...
cond_model.load_state_dict(torch.load(f"h_transform/num_cond_coeffs_{num_cond_coeffs}/h_transform.pt", weights_only=True))
...
def sample_conditional(x_gt, y, num_samples_K):
    ...
    for ti in tqdm(reversed(ts), total=len(ts), desc="Sampling", leave=False):
        ...
        cond_score_fourier = cond_score_flat.reshape(num_samples_K, 2, num_fourier_modes, 2)
