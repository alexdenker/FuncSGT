...
from sde import OU
from simple_network import CondScoreNet, ScoreNet
...
num_landmarks = 64
num_fourier_modes = (
    16  # Number of modes (will give 2*num_modes coefficients: pos + neg)
)
...
cond_model = CondScoreNet(
    input_dim=2 * num_fourier_modes * 2 + 2 * num_cond_coeffs * 2,
    output_dim=2 * num_fourier_modes * 2,
...
cond_model.load_state_dict(
    torch.load(
        f"h_transform/num_cond_coeffs_{num_cond_coeffs}/h_transform.pt",
        weights_only=True,
    )
)
...
def cond_model_fun(xt, t, cond):
    ...
    with torch.enable_grad():
        ...
        y_pred = xt_reshape[
            :, :, :num_cond_coeffs, :
        ]  # Extract predicted conditioning coefficients
        ...
        log_likelihood_grad = torch.autograd.grad(loss_y.sum(), xt, retain_graph=True)[
            0
        ].detach()
...
def sample_conditional(x_gt, y, num_samples_K):
    ...
    x_init = torch.sqrt(scales) * torch.randn(
        num_samples_K, 2, num_fourier_modes, 2, device=device
    )
    xt = x_init.clone()
    ...
    for ti in tqdm(reversed(ts), total=len(ts), desc="Sampling", leave=False):
        ...
        score_fourier = score_flat.reshape(num_samples_K, 2, num_fourier_modes, 2)
        cond_score_flat = cond_model_fun(xt_flat, t, y_batch)
        cond_score_fourier = cond_score_flat.reshape(
            num_samples_K, 2, num_fourier_modes, 2
        )
        score = score_fourier + cond_score_fourier
...
output_dir = Path(f"results/Htrans/num_cond_coeffs_{num_cond_coeffs}")
...
for dataset_idx in range(num_dataset_elements):
    ...
    x_fourier = fourier_coefficients(
        x_gt, num_bases=num_fourier_modes
    ...
    y_pred = fourier_coefficients(samples, num_bases=num_fourier_modes)[
        :, :, :num_cond_coeffs, :
    ]
    ...
    for sample_idx in range(num_samples):
        ...
        ax.plot(
            samples[sample_idx, :, 0].cpu().numpy(),
            samples[sample_idx, :, 1].cpu().numpy(),
            "-o",
            c="g",
            alpha=0.8,
            label="sample",
    ...
    axes[-1].plot(
        avg_sample[:, 0].cpu().numpy(),
        avg_sample[:, 1].cpu().numpy(),
        "-o",
        c="b",
        alpha=0.8,
        label="average",
