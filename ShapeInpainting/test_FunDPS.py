...
argparse = argparse.ArgumentParser(
    description="Conditional Sampling with Score-Based Model for Shape Data"
)

# argparse.add_argument('--mask_ratio', type=float, default=0.4, help='Fraction of points to mask')
argparse.add_argument(
    "--num_coeffs", type=int, default=5, help="Number of Fourier coefficients observed"
)
...
argparse.add_argument(
    "--part", type=str, default="test", help="Dataset part to use (train or test)"
)
...
def main(args):
    ...
    num_dataset_elements = (
        args.num_dataset_elements
    )  # Number of elements N to process from dataset

    # Create output directory for logging
    output_dir = Path(
        f"results/DPS/{part}/num_cond_coeffs_{num_cond_coeffs}/lambda_{lambd}"
    )
    ...
    if part == "train":
        dataset = MNISTShapesDataset(
            class_label=3, num_landmarks=num_landmarks, train=True
        )
    else:
        dataset = MNISTShapesDataset(
            class_label=3, num_landmarks=num_landmarks, train=False
        )
    ...
    for dataset_idx in range(num_dataset_elements):
        ...
        torch.manual_seed(
            dataset_idx
        )  # Use dataset_idx for reproducibility per element
        ...
        print(
            f"  - MSE (all points): mean={mse_all.mean():.6f}, std={mse_all.std():.6f}"
        )
        ...
        plt.tight_layout()
        plt.savefig(
            element_dir / "samples_visualization.png", dpi=100, bbox_inches="tight"
        )
        plt.close()
