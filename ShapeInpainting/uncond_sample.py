...
from utils import inverse_fourier, get_fourier_noise_scales
from sde import OU
...
device = "cuda"
num_fourier_modes = config["training"]["num_fourier_modes"]  # Number of modes (will give 2*num_modes coefficients: pos + neg)
...
fig, axes = plt.subplots(len(landmarks_list), batch_size_smpl, figsize=(3.5, 3.2), dpi=300)
fig.subplots_adjust(left=0.05, right=0.98, top=0.92, bottom=0.05, wspace=0.05, hspace=0.05)
...
for i, num_landmarks in enumerate(landmarks_list):
    ...
    for idx in range(batch_size_smpl):
        ...
        ax.plot(xt[idx, :, 0].cpu().numpy(), xt[idx, :, 1].cpu().numpy(), 'o-', 
               linewidth=0.8, markersize=2, color='#1f77b4')
        
        # Close the contour
        ax.plot([xt[idx, -1, 0].item(), xt[idx, 0, 0].item()], 
               [xt[idx, -1, 1].item(), xt[idx, 0, 1].item()], 'o-',
               linewidth=0.8, markersize=2, color='#1f77b4')
    ...
    axes[i, 0].text(-0.35, 0.5, landmark_labels[i], transform=axes[i, 0].transAxes,
                   fontsize=5, verticalalignment='center', rotation='vertical', weight='bold')
